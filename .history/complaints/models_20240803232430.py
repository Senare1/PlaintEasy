...
class Employees(models.Model):
    ...
    status = models.CharField(max_length=128,choices=EMP_STATUS,default="")
    sup_id = models.EmailField(unique=True)
    salary = models.IntegerField(default=0)
    address = models.CharField(max_length=128)
    email = models.EmailField(unique=True)
    lieu_naiss = models.CharField(max_length=128)
...
class Proof(models.Model):
    ...
    file = models.FileField(upload_to="complaints/proof/documents",validators=[validate_document],blank=True,null=True,verbose_name="Preuve en fichier")
        ...
        return self.image.name if self.image else (
            self.video.name if self.video else (
                self.audio.name if self.audio else (
                    self.file.name if self.file else 'No File'
                )
            )
        )
