...
from .forms import LoginEmailForm,LoginPhoneForm,SignupForm
...
def users(request):
    nb_total = CommonUser.objects.all().count()
    nb_customer = CommonUser.objects.filter(status="CLIENT").count()
    nb_citizen = CommonUser.objects.filter(status="CITOYEN").count()
...
def all_citozens(request):
    users = CommonUser.objects.filter(status="CITOYEN")
    context = {
        "users" : users,
    }
...
def all_customers(request):
    users = CommonUser.objects.filter(status="CLIENT")
    context = {
        "users" : users,
    }
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if user is not None:
                ...
                error_message.append("Identifiants invalides")
    else:
        form = LoginEmailForm()
    return render(request, 'accounts/login_email.html', {'form': form,'error_message': error_message})
...
def login_phone(request):
    ...
    if request.method == "POST":
        ...
        form = LoginPhoneForm()
    return render(request, 'accounts/login_phone.html', {'form': form,'error_message': error_message})
...
def login_email(request):
    if request.method == "POST":
        ...
        form = LoginEmailForm()
    return render(request,'complaints/login_email.html',{'form': form})
...
def login_phone(request):
    if request.method == "POST":
        ...
        form = LoginPhoneForm()
    return render(request,'complaints/login_phone.html',{'form': form})"""