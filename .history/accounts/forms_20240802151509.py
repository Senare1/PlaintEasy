...
class LoginEmailForm(forms.Form):
    email = forms.EmailField(
        widget=forms.EmailInput(attrs={'placeholder':'Email','class':'w-100 mx-auto border p-2 rounded pill'})
    )
    password = forms.CharField(
        widget=forms.PasswordInput(attrs={'placeholder':'Password','class':'border w-100 mx-auto p-2 rounded pill'})
    )

class LoginPhoneForm(forms.Form):
    phone = forms.CharField(
        widget=forms.TextInput(attrs={'placeholder':'Telephone','class':'w-100 mx-auto border p-2 rounded pill'})
    )
    password = forms.CharField(
        widget=forms.PasswordInput(attrs={'placeholder':'Password','class':'w-100 mx-auto border p-2 rounded pill'})
    )
