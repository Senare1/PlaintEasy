...
class SignupForm(UserCreationForm):
    full_name = forms.CharField(
        max_length=128,
        required=True,
        widget=forms.TextInput(attrs={'id': 'id_full_name','placeholder': 'exp: Valian Harouna','class':'w-75 border p-2 form-control rounded pill'})
    )
    phone = forms.CharField(
        max_length=15,
        required=True,
        widget=forms.TextInput(attrs={'id': 'id_phone','placeholder': 'exp: 76112245','name':'phone','class':'w-75 border p-2 form-control rounded pill'})
    )
    email = forms.EmailField(
        required=True,
        widget=forms.EmailInput(attrs={'id': 'id_email','placeholder': 'exp: clarisovle14@gmail.com','name':'email','class':'w-75 form-control border p-2 rounded pill'})
    )
