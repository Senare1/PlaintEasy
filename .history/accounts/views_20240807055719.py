    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if status:
                ...
                print(give_message(complaint.complainant,msg="treated"))
                # response = Response(
                ...
                print(give_message(complaint.complainant,msg="rejected"))
                # response = Response(
                #     complainant=complaint.complainant,
                #     complaint=complaint,
                ...
                #     f"D'apres une etude et une verification nous jugeons votre plainte pas bien fonde \n M(r) {complainant} merci de nous fournir de plus ample preuves",
                #     'arsenenikiema685@gmail.com',
                #     [complaint.complainant.email],
                #     fail_silently=False,
                # )
            else:
                print(give_message(complaint.complainant,msg="running"))
...
def treated(request):
    treated_complaints = Complaint.objects.all().filter(status="treated").count()
    context = {
        "treated_complaints" : treated_complaints,
    }
    return render(request,'accounts/rejection.html',context)
...
def running(request):
    running_complaints = Complaint.objects.all().filter(status="running").count()
    context = {
