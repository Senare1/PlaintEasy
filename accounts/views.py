...
def complaint_treated_or_rejected_detail(request, id):
    complaint = get_object_or_404(Complaint, id=id)
    complainant = complaint.complainant.full_name
    proofs = complaint.proofs.all()
    return render(request, 'accounts/complaint_treated_or_rejected_detail.html', context)
        ...
        if form.is_valid():
            ...
            if datetime.now() > expiration_time:
                ...
                if CommonUser.objects.filter(email=email).exists():
                    ...
                    del request.session['verification_code']
                    del request.session['code_expiration']
                    form.add_error(None,"Pas d'utilisateur correspondant")
                    return redirect('login_email')
            else:
                form.add_error('code', 'Code de vérification incorrect.')
    else:
...
def regenerate_email_code(request):
    if 'email' in request.session:
        ...
        send_mail(
                'Confirmer votre identité',
                f'Voici votre code de confirmation, svp ne le communiquez à personne : {verification_code}',
                'arsenenikiema685@gmail.com',
                [email],
                fail_silently=False,
            )
...
def change_password(request, email):
    ...
    return render(request, 'accounts/confirme.html', {'form': form})
